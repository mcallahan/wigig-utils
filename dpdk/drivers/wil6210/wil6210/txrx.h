...
static inline struct vring_rx_desc *wil_skb_rxdesc(struct rte_mbuf *skb)
{
	/* Use head space for RX meta */
	return (void *)skb->buf_addr;
}
...
static inline u8 *wil_skb_get_da(struct rte_mbuf *skb)
{
 	struct ethhdr *eth = rte_pktmbuf_mtod(skb, struct ethhdr *);

	return eth->h_dest;
}

static inline u8 *wil_skb_get_sa(struct rte_mbuf *skb)
{
 	struct ethhdr *eth = rte_pktmbuf_mtod(skb, struct ethhdr *);
...
static inline bool wil_need_txstat(struct rte_mbuf *skb)
{
#ifndef WIL6210_PMD
...
static inline void wil_consume_skb(struct rte_mbuf *skb, bool acked)
{
#ifndef WIL6210_PMD
...
#ifndef WIL6210_PMD
static inline u8 wil_skb_get_cid(struct rte_mbuf *skb)
{
