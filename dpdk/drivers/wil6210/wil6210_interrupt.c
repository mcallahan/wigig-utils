...
/*
 * UIO framework disables INTx interrupts when realying the
 * request to userland and userland is responsible to re-enable
 * them back. Do not bother with this is we know we are in MSI
 * mode as this call is a waste of cycles.
 */
static inline void wil6210_unmask_intx_int(struct wil6210_priv *wil)
{
	...
	pci_dev = wil->pdev;
#ifdef VFIO_PRESENT
	if (pci_dev->intr_handle.type == RTE_INTR_HANDLE_VFIO_MSI)
		return;
#endif
	ret = rte_intr_enable(&pci_dev->intr_handle);
	if (ret != 0)
		wil_err(wil, "unable to re-enable interrupts\n");
}
...
static void wil6210_interrupt_handler(void *data)
{
	struct wil6210_priv *wil = data;
	irqreturn_t rc;

	rc = wil6210_hardirq(0, wil);
	if (rc == IRQ_WAKE_THREAD)
		wil6210_thread_irq(0, data);
}

static int
wil6210_request_irq(struct wil6210_priv *wil)
{
	struct rte_pci_device *pci_dev;
	int rc;
	...
	rc = rte_intr_callback_register(&pci_dev->intr_handle,
					 wil6210_interrupt_handler,
					 wil);
	if (rc != 0) {
		wil_err(wil, "Failed to register interrupt callback, ret=%d\n",
			rc);
		return rc;
	}
...
void wil6210_fini_irq(struct wil6210_priv *wil, int irq)
{
	...
	rc = rte_intr_disable(&pci_dev->intr_handle);
	if (rc != 0) {
		wil_err(wil, "Failed to disable interrupt, ret=%d\n",
			rc);
		/* Continue anyway */
	}

	rc = rte_intr_callback_unregister(&pci_dev->intr_handle,
					 wil6210_interrupt_handler,
					 wil);
	if (rc <= 0) {
		wil_err(wil, "Failed to unregister interrupt callback, ret=%d\n",
			rc);
	}
#ifndef WIL6210_PMD
	free_irq(irq, wil);
