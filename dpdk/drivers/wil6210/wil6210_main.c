...
static int wil_find_pcie_cap(struct rte_pci_device *dev, u8 pos)
{
...
static int wil_get_pcie_params(struct wil6210_priv *wil, u16 *val)
{
	int ret, pos;
...
void wil_delete_peers(struct wil6210_priv *wil)
{
	...
	for (int i = 0; i < WIL6210_MAX_CID; i++) {
		...
		wil_api_link_down(wil, sta->peer_id, sta->addr);
		wil_eth_tx_ring_disable(wil, i, sta->peer_id);
		...
		/* release vrings */
		for (j = 0; j < ARRAY_SIZE(wil->ring_tx); j++) {
			if (wil->ring2cid_tid[j][0] == i)
				wil_ring_fini_tx(wil, j);
		}
...
int wil_priv_init(struct wil6210_priv *wil)
{
	...
	mutex_init(&wil->wmi_async_mutex);
	...
	char n[32];
	snprintf(n, sizeof(n), WIL_NAME "_wmi%u", wil->port_id);
	wil->wmi_wq = create_singlethread_workqueue(n);
	if (!wil->wmi_wq)
		return -EAGAIN;
...
void wil_get_board_file(struct wil6210_priv *wil, char *buf, size_t len)
{
	...
	prefix_len = (ext ? ext - board_file : (int)strlen(board_file));
...
{
	ulong to = msecs_to_jiffies(5000);
...
int wil_reset(struct wil6210_priv *wil, bool load_fw)
{
	...
	if (load_fw) {
		...
		rc = asprintf(&rfbb_board_file, "%s/%s",
			pdev->device.name, board_file);
		if (rc < 0) {
			wil_err(wil, "Error unable to build RFBB board file string");
			goto out;
		}
		wil_info(wil, "Using firmware <%s> + board file <%s>, <%s> \n",
...
void wil_fw_error_recovery(struct wil6210_priv *wil)
{
	...
	schedule_work(&wil->fw_error_worker, "wil-fw-error-recovery");
}
...
int __wil_up(struct wil6210_priv *wil)
{
	...
	if (wil->p2mp_capable) {
		...
		rc = wmi_set_pcie_config_params(wil, pcie_gen, pcie_lane_count);
		if (rc) {
