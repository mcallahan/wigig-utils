...
{
	void *fw_dump_data;
	u32 fw_dump_size, dump_size, pmc_dump_size;
	...
	dump_size = fw_dump_size + pmc_dump_size;
	fw_dump_data = vzalloc(dump_size);
	if (!fw_dump_data)
		return;
	...
	if (pmc_dump_size)
		wil_pmc_ext_copy_host_data(wil,
					   (char *)fw_dump_data + fw_dump_size,
					   pmc_dump_size);

	/* fw_dump_data will be free in device coredump release function
	 * after 5 min
	 */
	dev_coredumpv(wil_to_dev(wil), fw_dump_data, dump_size, GFP_KERNEL);
